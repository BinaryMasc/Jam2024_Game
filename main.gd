...
func story_flow_1():
	...
	sub_controller.new_subtitle("Ahora estoy en el baño.")
...
func story_flow_2():
	sub_controller.new_subtitle("¿Eh? ¿Quién dijo eso?")
	sub_controller.new_subtitle("Bueno, no estoy loco, seguro es por el agotamiento.")
	sub_controller.new_subtitle("Pero espera, se supone que esto es un \"descanso\".")
	sub_controller.new_subtitle_callback("¿Debería quedarme más tiempo aquí?", Callable(self, "story_flow_decision_2"))
...
func story_flow_4(option: int):
	if option == 1:
		...
		sub_controller.new_subtitle("Me quedo.")
